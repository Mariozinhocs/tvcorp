...
> **Status Atual:** 100% Operacional — Produção v1.0 / Go-to-Market  
> **Última Atualização:** 10 de Setembro de 2026  
> **Squad:** A-Team (PO: Mario Henrique / AI Agent: Antigravity AI)  
...
1. **Landing Page Pública ([`index.html`](file:///g:/Meu%20Drive/Dev's/TvCorp/index.html)):**
   ...
   * **Simulador de TV em Tempo Real (Live Studio CMS Simulator):** Player animado nativo em CSS/JS integrado ao banco de dados, com rotação de comunicados corporativos, relógio ao vivo, clima local e QR Codes promocionais.
...
2. **Central de Tutoriais & Ajuda ([`help.html`](file:///g:/Meu%20Drive/Dev's/TvCorp/help.html)):**
   * Guia 1: Instalação do Player em Samsung Tizen, LG WebOS, Android TV, Fire TV e TV Box.
   * Guia 2: Manual de pareamento em 6 dígitos sem senhas na TV.
   * Guia 3: Criação de playlists reativas (vídeos, imagens, clima tempo e widgets).
   * Guia 4: Gestão de pagamentos e assinaturas Mercado Pago.
   * Guia 5: **Como Usar Feeds RSS & Notícias ao Vivo** (G1, Folha, CNN, Canaltech e RSS interno).
   * Guia 6: **Modo Kiosk & AutoStart no Boot** (TV Box via Fully Kiosk / Launch on Boot, Samsung Tizen e LG webOS).
   * FAQ completo sobre execução contínua 24/7, memória e funcionamento offline.
3. **Área Privada Operacional do Assinante ([`app.html`](file:///g:/Meu%20Drive/Dev's/TvCorp/app.html)):**
   ...
   * Gestão de telas ativas com status em tempo real (🟢 Online / 🔴 Offline) e comandos remotos (Recarregar / Print).
4. **Painel Administrativo & Gestão SaaS ([`admin.html`](file:///g:/Meu%20Drive/Dev's/TvCorp/admin.html)):**
   ...
   * Ações administrativas em 1 clique: `+7d Trial`, `Alterar Plano`, `Suspender/Ativar Conta`.
   * Gestão de Anúncios e Banners do Simulador da Home (`api/admin.php?action=save_home_ad`).
...
7. **Relatórios de Status Executivo:**
   * Versão Markdown: [`RELATORIO_STATUS_EXECUTIVO.md`](file:///g:/Meu%20Drive/Dev's/TvCorp/RELATORIO_STATUS_EXECUTIVO.md)
   * Versão Microsoft Word: [`Relatorio_Status_Executivo_TvCorp.docx`](file:///g:/Meu%20Drive/Dev's/TvCorp/Relatorio_Status_Executivo_TvCorp.docx)
   * Versão Web Interativa: [`relatorio-status-executivo.html`](file:///g:/Meu%20Drive/Dev's/TvCorp/relatorio-status-executivo.html)
